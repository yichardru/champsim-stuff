...
/* Drishti enhancement 2 constants */
constexpr int D_SAMPLED_SETS_PER_SLICE = 16;
constexpr int LOG2_SAMPLED_SETS = log2(D_SAMPLED_SETS_PER_SLICE);
constexpr int D_LINES_PER_SLICE = LLC_SET * LLC_WAY;
constexpr int D_MONITOR_INTERVAL = 32768;
constexpr int D_RESELECT_INTERVAL = 131072;
constexpr int D_UNIFORM_THRESHOLD = 100;
constexpr int MPKA_COUNTER_MAX = 255;
...
/* Drishti: per-set MPKA tracking state */
int mpka_counter[LLC_SET];
bool is_d_sampled[LLC_SET]; // is part of sampled set selected by Drishti
int d_access_count; // global counter per LLC set
bool d_uniform_mode; // alternative for when mpka max - min < 100

/* Selecting top-MPKA sets as sampled sets */
void d_select_sampled_sets() {
    vector<pair<int, int>> ranked;
    ranked.reserve(LLC_SET);
    ...
    if (max_val - min_val < DRISHTI_UNIFORM_THRESHOLD) {
        d_uniform_mode = true;
        // Mark no sets as Drishti-selected
        fill(is_d_sampled, is_d_sampled + LLC_SET, false);
        return;
    }
    ...
    // Select top D_LINES_PER_SLICE sets.
    fill(is_d_sampled, is_d_sampled + LLC_SET, false);
    for (int i = 0; i < D_LINES_PER_SLICE && i < LLC_SET; i++) {
        is_d_sampled[ranked[i].second] = true;
    }
}

/* Resetting MPKA counters for the next interval */
void d_reset_counters() {
    fill(mpka_counter, mpka_counter + LLC_SET, 0);
}

bool is_sampled_set(int set) {
    if (d_uniform_mode) {
        // Use original static interleaved selection
...
void CACHE::llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t pc, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    // Saturating counter
    if (!hit) {
        mpka_counter[set] = min(mpka_counter[set] + 1, MPKA_COUNTER_MAX);
    }
    else {
        mpka_counter[set] = max(mpka_counter[set] - 1, 0);
    }

    d_access_count++;
 
    if (d_access_count == D_MONITOR_INTERVAL) {
        // Monitor window finished: perform initial high-MPKA set selection
        d_select_sampled_sets();
 
        // Allocate sampled cache entries for newly selected sets that weren't previously allocated
    ...
    if (d_access_count >= D_RESELECT_INTERVAL) {
        // Phase-change window done: reset counters and re-select
        d_reset_counters();
        d_select_sampled_sets();
        d_reset_counters = 0;
 
        // Allocate sampled cache entries for newly promoted sets
        for (uint32_t s = 0; s < LLC_SET; s++) {
            if (is_d_sampled[s]) {
                int modifier = 1 << LOG2_LLC_SET;
                int limit = 1 << LOG2_SAMPLED_CACHE_SETS;
